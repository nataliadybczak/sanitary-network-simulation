...
MIN_INTERVAL = 0.05  # Najszybciej (0.05s między krokami)
MAX_INTERVAL = 1.5  # Najwolniej (1.5s między krokami)
...
def map_window_loop(shared, lock, pause_evt, stop_evt, pos=(0, 50)):
    ...
    WIN_W, WIN_H = 900, 650  # Zwiększone nieco, żeby panel się ładnie mieścił
    # Margines na mapę, zostawiamy miejsce na dole na panel
    ...
    # Inicjalizacja stanu UI w shared (jeśli jeszcze nie ma)
    with lock:
        shared["map_scale"] = shared.get("map_scale", 1.0)
        shared["map_offset"] = shared.get("map_offset", (0, 0))
        shared["ui_slider_val"] = 0.5  # Połowa suwaka
        # Obliczamy startowy interwał na podstawie suwaka
    ...
    # Zmienne dla UI
    ui_elements = (None, None, None)  # btn_reset, btn_play, slider_rect
    ...
    while running and not stop_evt.is_set():
        ...
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                # Jeśli elementy UI zostały narysowane, sprawdzamy kliknięcie
                if ui_elements[0] is not None:
                    # ui_elements = (btn_reset, btn_play, slider_rect, handle)
                    handle_ui_click(event, ui_elements[0], ui_elements[1], ui_elements[2], shared, pause_evt)

            # Obsługa skrótu klawiszowego Spacja
            ...
            # Obsługa mapy (przesuwanie) - tylko jeśli NIE klikamy w panel UI
            # Zakładamy że panel jest poniżej MAP_ONLY_RECT
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        ...
        # Rysowanie paska kontrolnego na dole
        # Przekazujemy min/max interval żeby suwak dobrze działał
...
def chart_window_loop(shared, lock, pause_evt, stop_evt, pos=(980, 50)):
    ...
    while running and not stop_evt.is_set():
        ...
        # Wykrywanie RESETU (gdy godzina cofnęła się do 1, czyścimy wykresy)
        ...
        if pt is not None:
            ts, est, div = pt
            # Dodajemy punkt tylko jeśli jest nowy
...
# ====== Uruchomienie ======
def run_two_windows_dashboard(interval_sec: float = DEFAULT_INTERVAL):
    ...
    shared = manager.dict({
        ...
        "connections": [],
        "extra_points": [],
        "max_capacity": 1700,  # Wartość tymczasowa, nadpisze się z modelu
        "running": True,
        "hour": 0,
        "max_hours": 168,
        "reset_cmd": False,  # Komenda resetu
        "sim_interval": interval_sec,  # Aktualna prędkość
        "ui_slider_val": 0.5  # Pozycja suwaka
    })
    lock = manager.RLock()
    stop_evt = manager.Event()
    pause_evt = manager.Event()

    # Startujemy zapauzowani, żeby użytkownik mógł kliknąć Start
    pause_evt.set()

    # Definiujemy fabrykę modelu (funkcję która tworzy nowy model)
    # Możesz tu zmienić parametry (np. max_hours=168)
    def model_factory():
        return SewerSystemModel(max_capacity=2000, max_hours=168)

    # Tworzymy instancję modelu tylko po to, by pobrać wstępne max_capacity do shared
    # (Potem SimulationThread i tak stworzy swój własny model)
    temp_model = model_factory()
    shared["max_capacity"] = temp_model.max_capacity
    del temp_model

    # Wątek symulacji otrzymuje fabrykę, a nie instancję
